...
(() => {
  ...
  const legend='<span class="booked">Gebucht</span><span class="planned">Weitere Orte</span><span class="attention">Noch klären</span><span class="tip">Tipps</span><span class="gps">Dein Standort</span>';
  ...
  function renderMap(focusId){
    ...
    $('#map-results').innerHTML=`${points.length?points.map(p=>`<button class="map-result" data-focus="${p.id}"><span>${esc(p.name)}</span><small data-distance="${p.id}">${distanceLabel(p)}</small></button>`).join(''):'Keine Orte für diesen Filter.'}`;
    $('#map-filters').innerHTML=[['day',`${day.short} · Tagesorte`],['booked','Reserviert'],['included','E-Pass inklusive'],['discount','Mit Vergünstigung'],['tips','Tipps'],['all','Alle Orte']].map(([id,name])=>`<button data-map-filter="${id}" class="${mapFilter===id?'active':''}" aria-pressed="${mapFilter===id}">${name}</button>`).join('');
    $('#map-context').textContent=mapFilter==='tips'?'Für eure Reise vorgeschlagen: nach Interessen, Lage und verfügbarer Zeit ausgewählt. Quellen stehen bei jedem Ort.':mapFilter==='discount'?'E-Pass-Rabatte: Diese Angebote kosten zusätzlich. Preis und Verfügbarkeit beim Anbieter prüfen.':`${points.length} Kartenorte · Details und Treffpunkte durch Antippen öffnen.`;
  }
