...
(() => {
  ...
  const benefitLabels = {included:'E-Pass inklusive',free:'Frei zugänglich',discount:'Mit Vergünstigung',paid:'Separat bezahlt',booked:'Gebucht'};
  ...
  function initMap(){
    ...
    locationControl=window.createTripLocation({map,onState:s=>{
      $('#location-status').textContent=s.message;
  ...
  function renderMap(focusId){
    ...
    groups.forEach(group=>{
      ...
      const label=p.id==='hotel'?'H':group.length>1?group.length:(view==='plan'||mapFilter==='day')&&n?n:isFavorite(p.id)?'★':'•';
    ...
    $('#map-filters').innerHTML=[['day',`${day.short} · Tagesorte`],['booked','Reserviert'],['included','E-Pass inklusive'],['discount','Mit Vergünstigung'],['tips','Tipps'],['favorite','Favoriten'],['all','Alle Orte']].map(([id,name])=>`<button data-map-filter="${id}" class="${mapFilter===id?'active':''}" aria-pressed="${mapFilter===id}">${name}</button>`).join('');
    $('#map-context').textContent=mapFilter==='favorite'?`${points.length} Favoriten auf der Karte · Merkliste, noch keine Buchung. Abgleich unter Tagesplan oder Entdecken.`:mapFilter==='tips'?'Für eure Reise vorgeschlagen: nach Interessen, Lage und verfügbarer Zeit ausgewählt. Quellen stehen bei jedem Ort.':mapFilter==='discount'?'E-Pass-Rabatte: Diese Angebote kosten zusätzlich. Preis und Verfügbarkeit beim Anbieter prüfen.':`${points.length} Kartenorte · Details und Treffpunkte durch Antippen öffnen.`;
  }
  ...
  function renderInfo(){
    ...
    <article class="info-card"><p class="eyebrow">GEMEINSAME FAVORITEN</p><h3>Ein Plan für beide Geräte</h3><p>Mit demselben Reisecode verbinden sich beide Geräte mit eurer gemeinsamen Merkliste. Favoriten, übernommenes Planprofil und bis zu zehn vorherige Programmstände werden bei Supabase gespeichert. Der Abgleich erfolgt beim Öffnen, beim Zurückkehren und etwa alle 20 Sekunden.</p><p>Der Reisecode wird nur beim Anmelden übertragen. Auf dem Gerät bleibt ein zufälliger Sitzungsschlüssel, kein gespeicherter Reisecode. GPS-Positionen und Buchungsunterlagen werden nicht an diesen Speicher gesendet.</p><p>Offline gesetzte Häkchen warten sichtbar auf Übertragung. Das Tagesprogramm wird erst nach Vorschau und Übernahme geändert; mit „Zurück“ lässt sich der vorherige Stand für beide wiederherstellen. Favoriten sind keine Reservierungen.</p></article><article class="info-card"><p class="eyebrow">AUF DEM HANDY</p><h3>Als Webapp mitnehmen</h3><p><strong>iPhone:</strong> Die veröffentlichte HTTPS-Seite in Safari öffnen → Teilen → „Zum Home-Bildschirm“.</p><p><strong>Android:</strong> In Chrome öffnen → Menü → „App installieren“ oder „Zum Startbildschirm hinzufügen“.</p><button id="install-app" class="primary install" hidden>Webapp installieren</button><div class="install-note"><p>Nach dem ersten vollständigen Laden stehen Programm, Buchungen und Ortsdetails offline bereit. Die interaktive Kartenbasis, Live-Fahrpläne und externen Links brauchen Internet. Der E-Pass und seine QR-Codes bleiben im separaten Dashboard.</p><p id="offline-status" class="offline-status">Offline-Verfügbarkeit wird geprüft …</p></div></article>
  ...
  document.addEventListener('click',e=>{
    ...
    if(b.dataset.mapFilter){locationControl?.release();mapFilter=b.dataset.mapFilter;renderMap();}
  });
