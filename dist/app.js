...
(() => {
  ...
  function renderMap(focusId){
    ...
    $('#map-filters').innerHTML=[['day','Tagesorte'],['booked','Gebucht'],['included','E-Pass inklusive'],['discount','Mit Vergünstigung'],['favorite','Favoriten'],['tips','Tipps'],['all','Alle Orte']].map(([id,name])=>`<button data-map-filter="${id}" class="${mapFilter===id?'active':''}" aria-pressed="${mapFilter===id}">${name}</button>`).join('');
