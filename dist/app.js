...
(() => {
  ...
  let view='plan', mapFilter='day', map, markers, deferredInstall;
  const benefitLabels = {included:'E-Pass inklusive',free:'Frei zugänglich',discount:'E-Pass · Aufpreis',paid:'Separat bezahlt',booked:'Gebucht'};
  ...
  $('#intro-photo').innerHTML='<img src="./assets/istanbul-panorama.jpg" alt="Blick vom Galataturm über das Goldene Horn zur historischen Halbinsel" width="1280" height="427"><span class="photo-label">Goldenes Horn · Blick von Galata</span>';
  $('#open-issues').innerHTML='<div class="issue-strip"><span><strong>4 Punkte noch offen</strong><span class="issue-expanded"> · Kaffee ↔ Yacht, Ebru, Dinner-Abholung und Rücktransfer</span></span><button data-view="bookings">Prüfen ↗</button></div>';
  const legend='<span class="booked">Gebucht</span><span class="planned">Weitere Orte</span><span class="attention">Noch klären</span><span class="tip">Unsere Tipps</span>';
  ...
  function renderDay(){
    ...
    $('#itinerary').innerHTML=`<div class="day-header"><p class="eyebrow">${esc(day.short)} · ${esc(day.area)}</p><h2>${esc(day.title)}</h2><p class="day-subtitle">${esc(day.subtitle)}</p><div class="day-meta"><span class="mini-pill">${esc(day.pace)}</span><span class="mini-pill">Wege & Pausen eingeplant</span></div></div>${day.alert?`<details class="day-alert"><summary>${esc(day.id==='2026-10-04'?'Kaffee & Yacht: Anschluss vorab klären':day.id==='2026-10-06'?'Dinner-Abholung: Uhrzeit noch offen':'Führungszeiten am Vorabend prüfen')}</summary><p>${esc(day.alert)}</p></details>`:''}<div class="timeline">${day.events.map(e=>{
      const p=findPlace(e.place);
      return `<article class="event ${esc(e.status||'')}"><div class="event-time">${esc(e.time)}${e.end?`<small>– ${esc(e.end)}</small>`:''}</div><span class="event-dot" aria-hidden="true"></span><div class="event-card"><div class="event-top"><h3 class="event-title">${esc(e.title)}</h3>${e.label?badge(e.label,e.status||''):''}</div><p>${esc(e.text)}</p>${e.note?`<p class="small-note">${esc(e.note)}</p>`:''}${p?`<div class="event-actions"><button class="text-button" data-detail="${p.id}">Details</button><button class="text-button" data-map="${p.id}">Auf Karte</button>${external(mapsUrl(p),'Weg öffnen ↗')}</div>`:''}</div></article>${e.travel?`<p class="travel-note">↳ ${esc(e.travel)}</p>`:''}`;
    }).join('')}</div>`;
  ...
  function mapPlaces(){
    ...
    if(view==='plan' || mapFilter==='day') return TRIP.places.filter(p=>p.coords&&(ids.includes(p.id)||p.id==='hotel'));
  ...
  function renderMap(focusId){
    ...
    groups.forEach(group=>{
      ...
      const state=p.id==='hotel'?'hotel':p.status|| (p.tip?'tip':'');
      const ids=dayPlaceIds(); const n=ids.indexOf(p.id)+1;
      const label=p.id==='hotel'?'H':group.length>1?group.length:(view==='plan'||mapFilter==='day')&&n?n:'•';
    ...
    $('#map-results').innerHTML=`${points.length?points.map(p=>`<button class="map-result" data-focus="${p.id}">${esc(p.name)}</button>`).join(''):'Keine Orte für diesen Filter.'}`;
    $('#map-filters').innerHTML=[['day',`${day.short} · Tagesorte`],['booked','Reserviert'],['included','E-Pass inklusive'],['tips','Unsere Tipps'],['all','Alle Orte']].map(([id,name])=>`<button data-map-filter="${id}" class="${mapFilter===id?'active':''}" aria-pressed="${mapFilter===id}">${name}</button>`).join('');
  }
  ...
  function setView(next){
    ...
    (next==='map'?$('#map-large'):$('#map-home')).append($('#map'));
  ...
  function showDetail(id){
    const p=findPlace(id);if(!p)return;
    $('#place-detail').innerHTML=`<p class="eyebrow">${esc(p.area)} · ${esc(p.category)}</p><h2>${esc(p.name)}</h2><div class="detail-meta">${placeBadge(p)}${p.duration?badge(p.duration,'free'):''}${p.status?badge(p.status==='booked'?'Bestätigt':'Noch klären',p.status):''}</div><p>${esc(p.description)}</p>${p.notes?`<div class="detail-note"><p>${esc(p.notes)}</p></div>`:''}${p.hours?`<p><strong>Zeiten:</strong> ${esc(p.hours)}</p>`:''}${p.mode?`<p><strong>Zugang:</strong> ${esc(p.mode)}</p>`:''}<p class="muted">${esc(p.mapNote||'Kartenpunkt zur Orientierung. Exakten Eingang oder Treffpunkt vor dem Besuch prüfen.')}</p><div class="actions">${external(mapsUrl(p),'Ort / Treffpunkt öffnen ↗','primary')}${p.source?external(p.source,'Anbieter & aktuelle Details ↗','secondary'):''}${p.phone?`<a class="secondary" href="tel:${esc(p.phone)}">Anbieter anrufen</a>`:''}</div><p class="muted">Recherche: ${TRIP.checked}. E-Pass-Verfügbarkeit im persönlichen Dashboard prüfen. Angegebene Besuchs- und Wegzeiten sind Planungswerte.</p>`;
  ...
  function renderBookings(){
    $('#bookings').innerHTML=TRIP.bookings.map(b=>`<article class="booking-card"><p class="booking-date">${esc(b.date)}</p>${badge(b.label,b.status)}<h3>${esc(b.title)}</h3><div class="booking-time">${esc(b.time)}</div><p>${esc(b.text)}</p><p class="muted">Quelle: ${esc(b.source)}</p><div class="event-actions"><button class="text-button" data-detail="${b.place}">Details</button><button class="text-button" data-map="${b.place}">Auf Karte</button>${b.phone?`<a href="tel:${esc(b.phone)}">Anbieter anrufen ↗</a>`:''}</div></article>`).join('');
  }
  ...
  function renderDiscover(){
    ...
    if(filter==='tip')list=list.filter(p=>p.tip);
  ...
  function renderInfo(){
    ...
    <article class="info-card"><p class="eyebrow">NOCH ERLEDIGEN</p><h3>Vier offene Punkte</h3><ol><li>Kaffeedauer und Treffpunkt bestätigen; vorzugsweise auf Sonntag 14:00 Uhr verschieben lassen.</li><li>Ebru für Sonntag 13:00 Uhr reservieren, sofern verfügbar.</li><li>Für Dienstag die finale Dinnerbestätigung und genaue Hotelabholzeit besorgen.</li><li>Rücktransfer am Mittwoch vereinbaren; 06:35 Uhr ist der Planungswert.</li></ol><p>Diese Webseite hat keine Buchung geändert und keinen neuen Termin reserviert.</p><p>E-Pass-Support: <a href="tel:+908503023812">+90 850 302 3812</a></p></article>
    <article class="info-card wide"><p class="eyebrow">TRANSPARENTE PLANUNG</p><h3>Quellen & Aktualität</h3><p>Recherchestand: 21.09.2026. Reiseunterlagen, Hotelvoucher, Produktbeschreibung, deutschsprachiger Stadtführer und alle sieben vorhandenen E-Pass-Mails wurden mit dem persönlichen Dashboard und den aktuellen Anbieterseiten abgeglichen. Der ältere PDF-Stadtführer weicht bei manchen Leistungen und Öffnungen ab; aktuelle Detailseiten haben für Vorschläge Vorrang.</p><p>„Bestätigt“ beruht auf einer Bestätigungsmail bzw. den Reiseunterlagen. Die Dinnerfahrt erscheint im Dashboard als reserviert, ihre letzte vorliegende Mail wartet aber auf die finale Bestätigung. Alle weiteren Tageszeiten sind Vorschläge. Keine Live-Synchronisation mit dem E-Pass.</p><p>Der Katalog enthält ${window.EPASS_CATALOG.length} öffentlich gelistete Stadtattraktionen, einschliesslich klar markierter vorübergehender Schliessungen. Tages- und Mehrtagestouren sowie reine Zusatzservices sind ausgelassen. Das persönliche Dashboard bestimmt, was mit eurem konkreten Pass verfügbar ist. Karte: Anbieter-Kartenpunkte und Orientierungspunkte; einzelne Marker zeigen Treffpunkte statt Gebäude. Die drei Angebote ohne eindeutigen Punkt bleiben über ihre Anbieterlinks erreichbar.</p><div class="source-list">${external('https://istanbulepass.com/guided-tours-timetable','E-Pass-Führungszeiten ↗')}${external('https://istanbulepass.com/istanbul-attractions/','E-Pass-Katalog ↗')}${external('https://yourepass.com/reservations','Eure Reservierungen ↗')}${external('https://goturkiye.com/istanbul/kadikoy','Kadıköy · GoTürkiye ↗')}${external('https://goturkiye.com/istanbul/moda','Moda · GoTürkiye ↗')}</div><details><summary>Bildnachweis & technische Hinweise</summary><p>Foto: Juraj Patekar, <a href="https://commons.wikimedia.org/wiki/File:Wv_Istanbul_banner.jpg" target="_blank" rel="noopener">Wikimedia Commons</a>, <a href="https://creativecommons.org/licenses/by/2.0/" target="_blank" rel="noopener">CC BY 2.0</a>, im Layout zugeschnitten. Karte: OpenStreetMap-Mitwirkende; Kartenbibliothek Leaflet, BSD-2-Clause.</p><p>Keine Analyse- oder Werbetracker. Kartenkacheln werden von OpenStreetMap geladen; bei externen Links öffnet sich der jeweilige Anbieter. Reiseprogramm und öffentliche Anbieter-Kontakte sind Teil der Webseite. Buchungscodes, Passnummern, QR-Codes, Geburtsdaten und Originaldokumente werden nicht veröffentlicht.</p></details></article>`;
  }
  document.addEventListener('click',e=>{
    const b=e.target.closest('button,a');if(!b)return;
    if(b.dataset.view){e.preventDefault();setView(b.dataset.view);}
    if(b.dataset.day){day=TRIP.days.find(d=>d.id===b.dataset.day);renderDay();$('#plan-view').scrollIntoView({block:'start'});}
    if(b.dataset.detail)showDetail(b.dataset.detail);
    if(b.dataset.map)showOnMap(b.dataset.map);
    if(b.dataset.focus)renderMap(b.dataset.focus);
    if(b.dataset.mapFilter){mapFilter=b.dataset.mapFilter;renderMap();}
  });
  $('#map-expand').addEventListener('click',()=>{mapFilter='day';setView('map');});
