...
((root)=>{
  ...
  const alternatives={
    jash:[{day:1,from:'19:45',through:'19:45',events:[{...visit('jash','19:45','21:15','Nach der Yacht etwa 20–30 Minuten mit Reserve bergauf nach Cihangir gehen. Bei Jash armenisch geprägte Meze probieren.','Ersetzt das Abendessen bei Demeti. Tisch und Sonntagsöffnung anfragen; noch nicht reserviert. Bei später Yacht-Rückkehr einfach in Karaköy essen.'),status:'pause',travel:'Danach hinunter zur T1 und via Laleli zum Hotel; etwa 40–50 Minuten mit Fusswegen und Wartezeit.'}]}],
  ...
  function build(trip,favorites){
    ...
    const countsAsVisit=e=>e.status!=='pause'||known.get(e.place)?.restaurant;
    const selected=new Set(ids),original=new Set(days.flatMap(d=>d.events.filter(countsAsVisit).map(e=>e.place))),changes=[];
    ...
    for(const id of [...ids].sort((a,b)=>(DEFAULTS.indexOf(a)<0?100:DEFAULTS.indexOf(a))-(DEFAULTS.indexOf(b)<0?100:DEFAULTS.indexOf(b))||a.localeCompare(b))){
      ...
      for(const option of alternatives[id]||[]){
        ...
        if(keys.some(k=>occupied.has(k))||source.some(e=>e.flight||e.status==='booked'||BOOKED.includes(e.place)&&e.status!=='pause'||selected.has(e.place)&&countsAsVisit(e)))continue;
    ...
    for(const d of days){
      ...
      if(d.id==='2026-10-06'&&changes.some(c=>c.day===d.id&&c.added==='ep92')){const lunch=d.events.find(e=>e.time==='12:15');if(lunch){lunch.title='Mittagessen in Eminönü';lunch.text='In Eminönü in Ruhe etwas essen. Danach bleibt genug Zeit für die Fahrt nach Fener und Balat.';}}
    ...
    const planned=new Set(days.flatMap(d=>d.events.filter(countsAsVisit).map(e=>e.place)));
